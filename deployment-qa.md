...
## 2026-08-17 15:52:27Z — specification-aligned public route completion

Added dedicated static public routes `/features`, `/how-it-works`, and `/pricing` with a shared responsive Arabic information layout. The pages explain account/team, SaaS subscription and billing, link-only LMS connection, source-aware reports, support, security, onboarding, webhook-based activation, and transparent SaaS-only plan boundaries. Each page visibly states that Centralia does not create an LMS, copy its content, or directly read its database. Added `scripts/public-pages-smoke.sh` and registered `test:public-pages`; the smoke gate checks all three routes and the boundary message. The complete `pnpm test:regression-matrix` passed at epoch `1786981947`, including build output showing all three routes, API, public-page, degraded-plans, security, auth, edge, tenant, subscription, production-config, final-window, boundary, LMS-independence, collection-bound, and dependency checks. The real-time execution window remains active until epoch `1786999623` (`2026-08-17T20:47:03Z`).


## 2026-08-17T16:00:59Z — Public route discoverability correction

Completed a focused product-completion cycle after the specification route audit. The landing-page footer now uses Next.js internal links for `/contact`, `/resources/status`, `/privacy`, `/terms`, and `/refund-policy`, replacing placeholder `#contact` targets while preserving in-page product anchors. Redundant policy-page scaffolding was removed because the existing dynamic `[slug]` route already provides the authoritative contact and legal surfaces. The dedicated `/features`, `/how-it-works`, and `/pricing` pages remain active and are covered by the public-pages smoke gate.

Validation completed at the system-clock checkpoint above: `pnpm lint`, strict TypeScript validation with the documented SQLite URL, production build, and `pnpm test:regression-matrix` all passed. The matrix passed schema validation, seeded API smoke, public-pages smoke, degraded plans smoke, security, authentication, edge-case, tenant-isolation, subscription lifecycle, production configuration, final-window status, auth-boundary, LMS-independence, collection-bounds, and dependency audit checks. Generated `next-env.d.ts` metadata was removed from the working diff after validation. The real-time execution window remains active until `2026-08-17T20:47:03Z`.
...
## 2026-08-17T16:10:04Z — landing footer discoverability smoke coverage

Extended `scripts/public-pages-smoke.sh` to fetch the landing page and assert that its footer exposes `/contact`, `/resources/status`, `/privacy`, `/terms`, and `/refund-policy`, preventing future regressions to the completed public navigation wiring. The complete `pnpm test:regression-matrix` passed at the system-clock checkpoint above, including build, migration, seed, API, public-page, robots, sitemap, degraded-plans, security, authentication, edge-case, tenant-isolation, subscription lifecycle, production configuration, final-window, auth-boundary, LMS-independence, collection-bounds, and dependency checks. The real-time execution window remains active until `2026-08-17T20:47:03Z`.
...
## 2026-08-17 — Canonical production smoke gate
...
## 2026-08-17 — Full regression after exact-duration gate
