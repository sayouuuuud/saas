...
import { ArrowLeft, ArrowRight, Check, CircleHelp, Clock3, FileText, LifeBuoy, MessageSquare, RefreshCw, ShieldCheck } from 'lucide-react'
...
export default function SupportPage() {
  ...
  const loadTickets = useCallback(async () => {
    ...
    setTicketsError('')
    try {
      const response = await fetch('/api/tickets?limit=25', { signal: controller.signal })
  ...
  return (
    <main className="support-page">
      ...
      <section className="support-content section-container">
        ...
        <div className="tickets-section"><div className="tickets-heading"><div><h2>تذاكرك السابقة</h2><p>كل محادثاتك محفوظة في مكان واحد.</p></div><button type="button" className="button button-outline" aria-label="تصفية التذاكر"><FileText size={14} /> تصفية التذاكر</button></div><div className="tickets-table"><div className="ticket-row ticket-table-head"><span>رقم التذكرة</span><span>الموضوع</span><span>آخر تحديث</span><span>الحالة</span><span /></div>{ticketsError ? <div className="invoice-empty" role="alert"><span>{ticketsError}</span><button type="button" className="text-button" onClick={() => void loadTickets()} disabled={ticketsLoading}><RefreshCw size={14} /> إعادة المحاولة</button></div> : ticketsLoading ? <div className="invoice-empty" role="status">جارٍ تحميل التذاكر...</div> : tickets.length ? tickets.map((ticket) => <div className="ticket-row" key={ticket.id}><span className="ticket-id"><span className="ticket-id-icon"><MessageSquare size={13} /></span>{ticket.number}</span><span>{ticket.subject}</span><span>{new Date(ticket.updatedAt).toLocaleDateString('ar-EG')}</span><span className="ticket-status"><Check size={12} /> {ticket.status === 'OPEN' ? 'مفتوحة' : ticket.status}</span><button type="button" aria-label={`فتح ${ticket.number}`}><ArrowLeft size={14} /></button></div>) : <div className="invoice-empty">لا توجد تذاكر بعد.</div>}</div></div>
      </section>
    </main>
  )
}
