...
export default function SupportPage() {
  ...
  useEffect(() => {
    let active = true
    fetch('/api/tickets?limit=25')
      .then(async (response) => {
        const payload = await response.json().catch(() => ({}))
        if (!response.ok) throw new Error(payload.error || 'تعذر تحميل التذاكر السابقة')
        return payload
      })
      .then((payload) => { if (active) setTickets(payload.tickets || []) })
      .catch((loadError) => { if (active) setTicketsError(loadError instanceof Error ? loadError.message : 'تعذر تحميل التذاكر السابقة') })
