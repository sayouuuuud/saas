...
export default async function AdminSectionPage({ params, searchParams }: { params: Promise<{ section: string }>; searchParams: Promise<{ q?: string }> }) {
  ...
  if (section === 'teachers') {
    ...
    const rows = await prisma.user.findMany({ where: { isStaff: true, ...(query ? { OR: [{ name: { contains: query } }, { email: { contains: query } }] } : {}) }, take: 50, orderBy: { createdAt: 'desc' }, select: { id: true, name: true, email: true, emailVerifiedAt: true, createdAt: true } })
