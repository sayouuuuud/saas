    ...
    <main className="site-shell">
      ...
      <header className="site-header">
        ...
        <div className="header-actions">
          <a href="اديني login" className="login-link">تسجيل الدخول</a>
      ...
      <section id="how-it-works" className="workflow-section section-space"><div className="section-container workflow-inner"><div className="workflow-copy"><SectionEyebrow>كيف تعمل</SectionEyebrow><h2>ابدأ في دقائق،<br /><em>واعرف طريقك.</em></h2><p>لا إعدادات معقدة ولا وعود غامضة. خطوات قصيرة تأخذك من التسجيل إلى إدارة اشتراكك بكل وضوح.</p><Link href="/register" className="button button-dark">ابدأ الآن <ArrowLeft size={16} /></Link></div><div className="steps-list"><div className="step-item active"><span className="step-number">01</span><div><h3>أنشئ حسابك</h3><p>سجّل بياناتك التجارية وأنشئ مساحة عملك في مركزية.</p></div><CircleCheck size={19} /></div><div className="step-item"><span className="step-number">02</span><div><h3>اختر باقتك</h3><p>قارن المزايا والأسعار واختر ما يناسب مرحلتك الحالية.</p></div><span className="step-arrow">←</span></div><div className="step-item"><span className="step-number">03</span><div><h3>أضف رابط منصتك</h3><p>أدخل رابط الـ LMS الموجود لديك، أو افعل ذلك لاحقًا.</p></div><span className="step-arrow">←</span></div><div className="step-item"><span className="step-number">04</span><div><h3>أدر عملك بثقة</h3><p>تابع فواتيرك، فريقك، دعمك، وبياناتك من مكان واحد.</p></div><span className="step-arrow">←</span></div></div></div></section>

      <section id="pricing" className="pricing-section section-container section-space"><div className="section-heading centered"><SectionEyebrow>خطط تناسب مرحلتك</SectionEyebrow><h2>ابدأ صغيرًا،<br /><em>وتوسع بثقة.</em></h2><p>أسعار واضحة بلا حدود مخفية. كل الخطط تدير SaaS الخاص بك، ولا تدّعي امتلاك موارد LMS لا نقيسها.</p><div className="billing-toggle"><button type="button" className={!annual ? 'selected' : ''} aria-pressed={!annual} onClick={() => setAnnual(false)}>شهري</button><button type="button" className={annual ? 'selected' : ''} aria-pressed={annual} onClick={() => setAnnual(true)}>سنوي <span>وفر 20%</span></button></div></div><div className="plans-grid">{plans.map((plan) => <article className={`plan-card ${plan.featured ? 'featured-plan' : ''}`} key={plan.name}>{plan.featured && <div className="popular-pill">الأكثر اختيارًا</div>}<div className="plan-top"><div><span className="plan-name">{plan.arabicName}</span><small>{plan.name}</small></div><Sparkles size={19} /></div><p>{plan.description}</p><div className="price"><strong>${annual ? plan.yearly : plan.monthly}</strong><span>/ شهر</span></div>{annual && <div className="annual-note">تُدفع سنويًا · وفّر 20%</div>}<Link href="/register" className={`button ${plan.featured ? 'button-light' : 'button-outline'} plan-button`}>ابدأ مع {plan.arabicName} <ArrowLeft size={15} /></Link><div className="plan-divider" /><span className="includes">تشمل الخطة:</span><ul>{plan.features.map((feature) => <li key={feature}><Check size={15} /> {feature}</li>)}</ul></article>)}</div></section>
      ...
      <section id="faq" className="faq-section section-container section-space"><div className="faq-heading"><SectionEyebrow>أسئلة شائعة</SectionEyebrow><h2>وضوح قبل<br /><em>أن تبدأ.</em></h2><p>إذا لم تجد إجابتك، فريق الدعم جاهز لمساعدتك.</p><Link href="/contact" className="text-link">تواصل مع الدعم <ArrowLeft size={15} /></Link></div><div className="faq-list">{faqs.map((faq, index) => <div className={`faq-item ${openFaq === index ? 'open' : ''}`} key={faq.question}><button type="button" id={`faq-trigger-${index}`} aria-expanded={openFaq === index} aria-controls={`faq-panel-${index}`} onClick={() => setOpenFaq(openFaq === index ? -1 : index)}><span>{faq.question}</span><ChevronDown size={18} /></button>{openFaq === index && <p id={`faq-panel-${index}`}>{faq.answer}</p>}</div>)}</div></section>

      <section id="contact" className="cta-section section-container"><div className="cta-card"><div className="cta-pattern" /><div className="cta-copy"><SectionEyebrow>الخطوة التالية لك</SectionEyebrow><h2>خلّ إدارة منصتك<br /><em>أبسط من اليوم.</em></h2><p>أنشئ حسابك في دقائق، وابدأ برؤية كل ما يخص SaaS الخاص بك في مكان واحد.</p><Link href="/register" className="button button-light button-large">ابدأ مجانًا <ArrowLeft size={18} /></Link></div><div className="cta-orbit"><div className="cta-orbit-ring ring-a" /><div className="cta-orbit-ring ring-b" /><div className="cta-orbit-core"><BrandMark /><span>مركزية</span></div><div className="orbit-tag tag-a"><CircleCheck size={14} /> اشتراك نشط</div><div className="orbit-tag tag-b"><Link2 size={14} /> رابط LMS</div><div className="orbit-tag tag-c"><BarChart3 size={14} /> تقارير واضحة</div></div></div></section>

      <footer className="site-footer section-container"><div className="footer-main"><div className="footer-brand"><Link href="/" className="brand"><BrandMark /><span>مركزية</span></Link><p>مساحة واحدة لإدارة اشتراكك ومنصتك التعليمية بوضوح.</p></div><div className="footer-links"><div><b>المنتج</b><a href="#features">المزايا</a><a href="#how-it-works">كيف تعمل</a><a href="#pricing">الأسعار</a></div><div><b>المساعدة</b><a href="#faq">الأسئلة الشائعة</a><Link href="/resources">المصادر</Link><Link href="/use-cases">حالات الاستخدام</Link><Link href="/contact">تواصل معنا</Link><Link href="/resources/status">حالة النظام</Link></div><div><b>قانوني</b><Link href="/privacy">الخصوصية</Link><Link href="/terms">الشروط</Link><Link href="/refund-policy">سياسة الاسترداد</Link><Link href="/acceptable-use">الاستخدام المقبول</Link></div></div></div><div className="footer-bottom"><span>© 2025 مركزية. جميع الحقوق محفوظة.</span><span className="footer-note"><Clock3 size={14} /> مبنية للوضوح، لا للتعقيد.</span></div></footer>
    </main>
  )
}
