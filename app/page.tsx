    ...
    <main className="site-shell">
      ...
      <header className="site-header">
        ...
        <div className="header-actions">
          ...
          <button className="menu-toggle" onClick={() => setMobileMenuOpen((value) => !value)} aria-label="فتح القائمة">
      ...
      <section id="faq" className="faq-section section-container section-space"><div className="faq-heading"><SectionEyebrow>أسئلة شائعة</SectionEyebrow><h2>وضوح قبل<br /><em>أن تبدأ.</em></h2><p>إذا لم تجد إجابتك، فريق الدعم جاهز لمساعدتك.</p><a href="#contact" className="text-link">تواصل مع الدعم <ArrowLeft size={15} /></a></div><div className="faq-list">{faqs.map((faq, index) => <div className={`faq-item ${openFaq === index ? 'open' : ''}`} key={faq.question}><button onClick={() => setOpenFaq(openFaq === index ? -1 : index)}><span>{faq.question}</span><ChevronDown size={18} /></button>{openFaq === index && <p>{faq.answer}</p>}</div>)}</div></section>
