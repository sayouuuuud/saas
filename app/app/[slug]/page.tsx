...
export default async function AppWorkspacePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const counts = user.workspace ? await Promise.all([prisma.invoice.count({ where: { workspaceId: user.workspace.id } }), prisma.supportTicket.count({ where: { workspaceId: user.workspace.id } }), prisma.lmsLink.count({ where: { workspaceId: user.workspace.id } })]) : [0, 0, 0];
