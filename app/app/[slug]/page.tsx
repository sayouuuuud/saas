...
export default async function AppWorkspacePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [counts, teamMembers] = await Promise.all([
    Promise.all([prisma.invoice.count({ where: { workspaceId: user.workspace.id } }), prisma.supportTicket.count({ where: { workspaceId: user.workspace.id } }), prisma.lmsLink.count({ where: { workspaceId: user.workspace.id } })]),
    slug === "team" ? prisma.workspaceMember.findMany({ where: { workspaceId: user.workspace.id }, orderBy: { createdAt: "asc" }, take: 26, include: { user: { select: { id: true, name: true, email: true } } } }) : Promise.resolve([]),
  ]);
  const teamHasMore = teamMembers.length > 25;
  return <WorkspaceShell active={slug} title={data.title} intro={data.intro}><section className="workspace-card-grid"><article className="workspace-panel workspace-panel-accent"><span className="section-eyebrow"><span className="eyebrow-dot" />{data.heading}</span><h2>{data.heading}</h2><p>{data.body}</p><ul className="workspace-bullets">{data.bullets.map((bullet) => <li key={bullet}><CircleCheck size={15} />{bullet}</li>)}</ul></article><article className="workspace-panel"><div className="workspace-panel-heading"><b>مؤشرات مساحة العمل</b><ExternalLink size={16} /></div><div className="workspace-stat-list"><div><span>الفواتير</span><strong>{counts[0]}</strong></div><div><span>التذاكر</span><strong>{counts[1]}</strong></div><div><span>روابط LMS</span><strong>{counts[2]}</strong></div></div><p className="safe-note"><ShieldCheck size={15} /> مؤشرات SaaS فقط — لا قراءة لمحتوى المنصة.</p></article></section>{slug === "team" && <TeamMembersPanel initialMembers={teamMembers.slice(0, 25)} initialNextOffset={teamHasMore ? 25 : null} />}<section className="workspace-next"><Link href="/billing" className="button button-dark">فتح الفوترة</Link><Link href="/support" className="button button-outline">فتح الدعم</Link><Link href="/dashboard" className="text-link">لوحة التحكم الحالية ↩</Link></section></WorkspaceShell>;
}
