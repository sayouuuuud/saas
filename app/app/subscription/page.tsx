      ...
      <section className="billing-content section-container">
        ...
        {loading ? <div className="invoice-empty" role="status">جارٍ تحميل بيانات الاشتراك...</div> : subscription ? <>
          <div className="billing-current-grid">
            ...
            <article className="payment-card"><div className="billing-card-title"><span>ملخص الفترة</span><CalendarDays size={17} /></div><div className="payment-method"><div><b>بدأ الاشتراك</b><small>{formatDate(subscription.startedAt)}</small></div></div><div className="payment-method"><div><b>بداية الفترة الحالية</b><small>{formatDate(subscription.currentPeriodStart)}</small></div></div><div className="payment-method"><div><b>نهاية الفترة الحالية</b><small>{formatDate(subscription.currentPeriodEnd)}</small></div></div><div className="payment-method"><div><b>دورة الفوترة</b><small>{subscription.billingCycle === 'YEARLY' ? 'سنوية' : 'شهرية'}</small></div></div><div className="payment-note"><ShieldCheck size={13} /> لا نخزن بيانات البطاقة</div></article>
          </div>
