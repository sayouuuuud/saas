...
export default function NotificationsPage() {
  ...
  const [preferences, setPreferences] = useState({ emailEnabled: true, productEnabled: true, billingEnabled: true })
  ...
  const load = useCallback(async (offset = 0, append = false) => {
    ...
    try {
      ...
      if (preferencesResponse.ok && preferencesPayload.preferences) setPreferences(preferencesPayload.preferences)
    } catch (loadError) {
  ...
  async function savePreferences(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch('/api/notifications/preferences', { method: 'PATCH', cache: 'no-store', headers: { 'content-type': 'application/json' }, body: JSON.stringify(preferences) })
      const payload = await response.json().catch(() => ({}))
      if (!response.ok) throw new Error(payload.error || 'تعذر حفظ تفضيلات الإشعارات')
      setError('تم حفظ تفضيلات الإشعارات.')
    } catch (saveError) { setError(saveError instanceof Error ? saveError.message : 'تعذر حفظ تفضيلات الإشعارات') } finally { setPreferencesBusy(false) }
  }
