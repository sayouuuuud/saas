...
export async function POST(_request: Request, context: Context) {
  ...
  const existingRequest = await prisma.auditLog.findFirst({ where: { workspaceId: user.workspace.id, action: "CREATE", entity: "IntegrationRequest", entityId: id, reason: "teacher requested future integration; no external call made" }, select: { id: true } });
  if (existingRequest) return NextResponse.json({ status: "already_recorded", message: "تم تسجيل طلب التكامل لهذا الرابط مسبقًا. لن يبدأ أي تكامل قبل توفير API رسمي وموافقة منفصلة." }, { status: 200 });
