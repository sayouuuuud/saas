...
export async function POST(request: Request) {
  try {
    ...
    if (!user || !valid) return NextResponse.json({ error: "البريد أو كلمة المرور غير صحيحة" }, { status: 401, headers });
    ...
    if (user.twoFactorEnabled) {
      ...
      if (!challenge || challenge.userId !== user.id || challenge.expiresAt <= new Date() || challenge.attempts >= 5 || !user.twoFactorSecretEncrypted) return NextResponse.json({ error: "جلسة التحقق منتهية، ابدأ تسجيل الدخول من جديد" }, { status: 401, headers });
