...
function isPrivateIpv4(ip: string) {
  ...
  return parts[0] === 0 || parts[0] === 10 || parts[0] === 100 && parts[1] >= 64 && parts[1] <= 127 || parts[0] === 127 || parts[0] === 169 && parts[1] === 254 || parts[0] === 172 && parts[1] >= 16 && parts[1] <= 31 || parts[0] === 192 && parts[1] === 0 && parts[2] === 0 || parts[0] === 192 && parts[1] === 0 && parts[2] === 2 || parts[0] === 192 && parts[1] === 168 || parts[0] === 198 && (parts[1] === 18 || parts[1] === 19 || parts[1] === 51) || parts[0] === 203 && parts[1] === 0 && parts[2] === 113;
}
...
export async function validateExternalHttpsUrl(value: string) {
  ...
  if (url.username || url.password) return { ok: false as const, reason: "credentials_forbidden" };
  if (url.port && url.port !== "443") return { ok: false as const, reason: "port_forbidden" };
  if (hostname === "localhost" || hostname.endsWith(".localhost") || hostname.endsWith(".local") || hostname.endsWith(".internal") || hostname.endsWith(".home.arpa") || hostname.endsWith(".test") || hostname.endsWith(".invalid") || hostname.endsWith(".example") || isPrivateIp(hostname)) return { ok: false as const, reason: "private_host" };
  try {
