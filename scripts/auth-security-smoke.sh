...
register="$(curl -fsS -c "$COOKIES" -H 'Content-Type: application/json' -d "{\"name\":\"Auth QA\",\"email\":\"$EMAIL\",\"password\":\"StrongPass123!\"}" "$BASE_URL/api/auth/register")"
...
verify="$(curl -fsS -H 'Content-Type: application/json' -d "{\"token\":\"$token\"}" "$BASE_URL/api/auth/verify")"
printf '%s' "$verify" | grep -q '"verified":true'
forgot="$(curl -fsS -H 'Content-Type: application/json' -d "{\"email\":\"$EMAIL\"}" "$BASE_URL/api/auth/forgot-password")"
...
reset="$(curl -fsS -H 'Content-Type: application/json' -d "{\"token\":\"$reset_token\",\"password\":\"NewStrongPass123!\"}" "$BASE_URL/api/auth/reset-password")"
