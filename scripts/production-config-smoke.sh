...
DATABASE_URL='postgresql://centralia:secret@example.com:5432/centralia' \
...
node "$ROOT_DIR/scripts/verify-production-config.mjs" >/tmp/saas-production-config-no-tls.log 2>&1; then
  echo 'Expected PostgreSQL URL without TLS mode to fail' >&2
  exit 1
fi
grep -q 'sslmode' /tmp/saas-production-config-no-tls.log
