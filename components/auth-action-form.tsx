...
export default function AuthActionForm({ mode }: { mode: Mode }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      setStatus({ type: "error", text: "تعذر الاتصال بالخدمة. حاول مرة أخرى." });
    }
  }
  const title = mode === "verify" ? "تحقق من بريدك" : mode === "forgot" ? "استعادة كلمة المرور" : "تعيين كلمة مرور جديدة";
  const description = mode === "verify" ? "أدخل رمز التحقق الذي وصلك لإكمال إعداد الحساب." : mode === "forgot" ? "سنقبل الطلب دون كشف ما إذا كان البريد مسجلًا." : "استخدم رمزًا صالحًا واختر كلمة مرور من 6 أحرف على الأقل.";
  return <main className="auth-action-page"><div className="auth-action-card"><Link href="/" className="brand"><span className="brand-mark"><span /><span /><span /></span>مركزية</Link><span className="section-eyebrow"><span className="eyebrow-dot" />أمان الحساب</span><h1>{title}</h1><p>{description}</p><form onSubmit={submit}>{mode !== "verify" && mode !== "reset" && <label>البريد الإلكتروني<input type="email" name="email" autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} required /></label>}{(mode === "verify" || mode === "reset") && <label>رمز الأمان<input name="token" autoComplete="one-time-code" value={token} onChange={(event) => setToken(event.target.value)} required /></label>}{mode === "reset" && <label>كلمة المرور الجديدة<input type="password" name="password" autoComplete="new-password" minLength={6} value={password} onChange={(event) => setPassword(event.target.value)} required /></label>}<button type="submit" className="button button-dark" disabled={status.type === "loading"}>{status.type === "loading" ? "جارٍ التنفيذ…" : "متابعة"}</button></form>{status.text && <p className={`auth-action-status ${status.type}`} role={status.type === "error" ? "alert" : "status"} aria-live="polite">{status.text}</p>}<div className="auth-action-links"><Link href="/login">تسجيل الدخول</Link><Link href="/contact">التواصل مع الدعم</Link></div></div></main>;
}
