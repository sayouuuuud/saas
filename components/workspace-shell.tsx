...
type WorkspaceShellProps = { active: string; title: string; intro: string; children: React.ReactNode };

export default async function WorkspaceShell({ active, title, intro, children }: WorkspaceShellProps) {
  const user = await getCurrentUser();
  const displayName = user?.name || "م";
  const workspaceName = user?.workspace?.name || "مساحة العمل";
  const email = user?.email || "حساب SaaS";
